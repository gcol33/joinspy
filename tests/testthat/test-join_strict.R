...
test_that("join_strict fails with 1:n when expecting 1:1", {
  x <- data.frame(id = 1:3, val = c("a", "b", "c"))
...
test_that("join_strict fails with n:1 when expecting 1:1", {
...
test_that("join_strict succeeds with 1:n when y has duplicates", {
  x <- data.frame(id = 1:3, val = c("a", "b", "c"))
  y <- data.frame(id = c(1, 1, 2, 3), name = c("A1", "A2", "B", "C"))
...
test_that("join_strict fails with 1:n when x has duplicates", {
  x <- data.frame(id = c(1, 1, 2), val = c("a", "b", "c"))
  y <- data.frame(id = 1:3, name = c("A", "B", "C"))

  expect_error(
    join_strict(x, y, by = "id", expect = "1:n"),
    "Cardinality violation"
  )
})

test_that("join_strict succeeds with n:1 when x has duplicates", {
  x <- data.frame(id = c(1, 1, 2, 3), val = c("a", "b", "c", "d"))
  y <- data.frame(id = 1:3, name = c("A", "B", "C"))
...
test_that("join_strict fails with n:1 when y has duplicates", {
  x <- data.frame(id = 1:3, val = c("a", "b", "c"))
  y <- data.frame(id = c(1, 1, 2, 3), name = c("A1", "A2", "B", "C"))

  expect_error(
    join_strict(x, y, by = "id", expect = "n:1"),
    "Cardinality violation"
  )
})

test_that("join_strict accepts n:m with any cardinality", {
  x <- data.frame(id = c(1, 1, 2), val = c("a", "b", "c"))
...
test_that("join_strict accepts alternative cardinality syntax", {
  ...
  # "1:many" should work same as "1:n"
  result <- join_strict(x, y, by = "id", expect = "1:many")
  expect_equal(nrow(result), 4)

  # "many:many" should work same as "n:m"
